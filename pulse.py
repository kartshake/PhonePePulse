...
class data_load:
    ...
    def sql_table_creation():
        con = psycopg2.connect(host='localhost', user='postgres', password='root', database='phonepe')
        cursor = con.cursor()
...
class plotly:
    ...
    def top10_transaction_pincode_vertical_bar_chart(df, x, y, text, color, title, title_x=0.25):
        ...
        fig.update_layout(title_x=title_x, title_font_size=22)
        #new_text_xaxis = [str(pincode).replace('@@@', '<br>') if not pd.isna(pincode) and not isinstance(pincode, float) else '' for pincode in df[x]]
    ...
    def top10_user_vertical_bar_chart(df, x, y, text, color, title, title_x=0.25):
        ...
        fig = px.bar(df, x=x, y=y, labels={x: '', y: ''}, title=title)
        fig.update_layout(title_x=title_x, title_font_size=22)
        #new_text_xaxis = [str(user).replace('@@@', '<br>') if not pd.isna(user) and not isinstance(user, float) else '' for user in df[x]]
